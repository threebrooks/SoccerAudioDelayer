...
class AudioDelayer:
    ...
    def pusherFunc(self, output_sampling_rate, bytes_per_sample, audio_chunk_size):
        self.audio_play = subprocess.Popen(['aplay','-D','default','-c','2','-t','raw','-r',str(output_sampling_rate),'-f','S16_LE','-'],stdin=subprocess.PIPE) 
        while (self.running.value):
...
if __name__ == '__main__':
    try:
        audio_delayer = AudioDelayer(stream_url='http://stream.antenne.def:80/antenne')
